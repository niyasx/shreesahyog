...
class LrGrPrinting extends StatefulWidget {
  const LrGrPrinting({super.key});

  @override
  State<LrGrPrinting> createState() => _LrGrPrinting();
}

class _LrGrPrinting extends State<LrGrPrinting> {
  final control = Get.put(Controller());
  RxBool loaderScreen = false.obs;

  List<LrGrTransportResponseList> originaltransporterBidList = [];
  ValueNotifier<List<LrGrTransportResponseList>> transporterbidtResponseList =
      ValueNotifier([]);
  ValueNotifier<List<LrGrTransportResponseList>> transporterbidtResponseList1 =
      ValueNotifier([]);
  ...
  List<LrGrTransportResponseList> getCurrentPageItems(
      List<LrGrTransportResponseList> items, int currentPage) {
    transporterbidtResponseList.value = items;
    final startIndex = (currentPage - 1) * pageSize;
    final endIndex = startIndex + pageSize > items.length
        ? items.length
        : startIndex + pageSize;
    List<LrGrTransportResponseList> a = items.sublist(startIndex, endIndex);
  ...
  fetchLrGrList() async {
    ...
    final LrGrTransporterModel response =
        await LrGrPrintingAPI().getLrGrData(context);
  ...
  void filterData(String searchTerm) {
    ...
    List<LrGrTransportResponseList> filteredData = originaltransporterBidList
        .where((value) =>
            value.plantId
                .toString()
                .toLowerCase()
                ...
                children: [
                  ...
                  ValueListenableBuilder(
                    ...
                    builder: (BuildContext context,
                        List<LrGrTransportResponseList> value, Widget? child) {
                      if (value.isEmpty) {
                        return const Center(
                          child: Padding(
                            ...
                            return Row(
                              children: [
                                ...
                                column(
                                    ...
                                    colColor: clr,
                                    columnTitle:
                                        value[index].invoiceCount.toString(),
                        ...
                        child: Row(
                          children: [
                            ValueListenableBuilder(
                              ...
                              builder: (BuildContext context,
                                  List<LrGrTransportResponseList> value,
                                  Widget? child) {
                                if (value.isEmpty) {
                                  return Container();
                                } else {}
  ...
  ValueListenableBuilder<List<LrGrTransportResponseList>>
      buildValueListenableBuilder() {
    return ValueListenableBuilder(
      valueListenable: transporterbidtResponseList,
      builder: (BuildContext context, List<LrGrTransportResponseList> value,
          Widget? child) {
        if (value.isEmpty) {
