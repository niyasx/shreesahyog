...
import '../../global.dart';
import '../../main.dart';
import '../biddingProcess/api/reports_api.dart';
...
class BidReportsState extends State<BidReports> {
  ...
  Future<dynamic> fetchPlantList() async {
    try {
      final ProfileModel response = await _getProfileList();
      List<ProfileResponseList> originalProfileList = response.responseList ?? [];
      for (int i = 0; i < originalProfileList.length; i++) {
        avlList[originalProfileList[i].plantName.toString()] =
            originalProfileList[i].plantCode??0;
      }
      return true;
    } catch (e) {}
  ...
  Widget build(BuildContext context) {
    ...
    builder: (stateIndex, snapshot) {
      if (snapshot.connectionState ==
        ...
        // selectedPlant.value = avlList.entries.first;
        // // Ensure avlList is not empty before using .first
        if (avlList.isNotEmpty) {
          selectedPlant.value = avlList.entries.first;
        } else {
          selectedPlant.value = const MapEntry("", 0); // Safe default value
        }
